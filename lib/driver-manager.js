...
export function startBrowser( { useCustomUA = true, resizeBrowserWindow = true } = {} ) {
	...
	if ( config.has( 'sauce' ) && config.get( 'sauce' ) ) {
		...
		const sauceURL = 'http://162.222.75.33:80/wd/hub';
	...
	driver
		.manage()
		.timeouts()
		.implicitlyWait( webDriverImplicitTimeOutMS );
	driver
		.manage()
		.timeouts()
		.pageLoadTimeout( webDriverPageLoadTimeOutMS );
